...
class Validate
{
	...
	{
		if (isset($this->errors[$element]) && !empty($this->errors[$element])) {
			$this->errors[$element] .= "<br>-" . $message;
		} else {
			...
			if ($value['required'] == true && trim($this->source[$element]) == null) {
				...
				switch ($value['type']) {
					...
					case 'existRecord':
						...
						break;
					case 'selectBox':
						$this->validateSelectBox($element, $value['options']["invalid"]);
						break;
				}
			}
	...
	{
		$length = strlen(trim($this->source[$element]));
		if ($length < $min) {
	...
	// Validate SelectBox 
	private function validateSelectBox($element, $invalidValue)
	{
		if ($this->source[$element] == $invalidValue || strval($this->source[$element]) == $invalidValue) {
			$this->setError($element, "must not be '" . $invalidValue . "'");
		}
	}
	...
	public function showErrorsAdmin()
	{
		...
		if (!empty($this->errors)) {
			...
			foreach ($this->errors as $key => $value) {
				$xhtml .= '<li class="text-white"><b>' . $key . ':</b> ' . $value . '</li>';
			}
			$xhtml .= '</ul>';
		}
		$xhtml .=  '</div>';
